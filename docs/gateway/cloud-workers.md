...
Codex remote execution additionally requires `settings.setup` or the cloud image to install the exact official `@openclaw/codex` plugin matching the Gateway version, including its pinned platform-native `@openai/codex` dependency. Enrollment validates the trusted installed plugin and projects it into the node's isolated per-lease state; it does not install plugins from npm or accept an untrusted local plugin path. A missing, mismatched, or untrusted plugin fails before the node starts.
